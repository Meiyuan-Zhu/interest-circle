import { Body, Config, Controller, Fields, File, Get, Inject, Param, Post, Put } from "@midwayjs/core";
...
export class PostController {
  ...
  async getUserNotifications(@Param('username') username: string) {
    try {
      ...
      this.ctx.body = { success:  true, notifications: user.notifications };

    } catch (error) {
      console.error('Error fetching user notifications:', error);
      this.ctx.body = { success: false, message: 'Error fetching user notifications' };
    }
  }

  @Put('/:username/notifications/:notificationId/read')
  async markNotificationAsRead(@Param('username') username: string, @Param('notificationId') notificationId: string) {
    try {
      await this.postService.markNotificationAsRead(username, notificationId);
